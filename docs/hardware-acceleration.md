...
- All configured GPU backends on a hardware runner:
  ...
  The hardware profile prints `atlas doctor` adapter diagnostics, emits
  placement-selected and per-SDK forced-GPU benchmarks with `speedup_ratio`, then
  runs real-device tests, attempts every configured backend, and reports all
  backend failures together at the end.
- OpenCL:
