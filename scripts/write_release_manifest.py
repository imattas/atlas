...
def validate_manifest(path: Path) -> list[str]:
    ...
    if not CTF_BENCHMARK_EVIDENCE.issubset(benchmark_paths):
        errors.append("CTF benchmark evidence is incomplete")
...
def render_manifest() -> str:
    ...
    for name, path, hardware, sample in [
        ...
        (
            "ctf-multi-family-benchmarks",
            "benchmarks/ctf/manifest.toml",
            "local CPU plus optional solver baselines",
            "xor, checksum, rotate-xor, modular arithmetic, serial-byte and native math CTF kernels",
        ),
        (
            "ctf-benchmark-results",
            "benchmarks/results/ctf-benchmarks.json",
            "local CPU plus optional solver baselines",
            "measured current CTF benchmark run generated by benchmarks/compare_external.py --write",
        ),
    ]:
        body.extend(
            [
