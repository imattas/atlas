...
#[derive(Debug)]
struct CancellingSuccessfulRunner {
    token: CancellationToken,
}

impl DriverRunner for CancellingSuccessfulRunner {
    fn run(&self, _plan: &DriverCommandPlan) -> DriverRunOutput {
        self.token.cancel();
        DriverRunOutput {
            exit_code: 0,
...
#[derive(Debug)]
struct CancellingEmptyRunner {
    token: CancellationToken,
}

impl DriverRunner for CancellingEmptyRunner {
    fn run(&self, _plan: &DriverCommandPlan) -> DriverRunOutput {
        self.token.cancel();
        DriverRunOutput {
            exit_code: 0,
            reported_matches: Vec::new(),
            stdout: "device completed\n".to_owned(),
            stderr: String::new(),
        }
    }
}
...
#[test]
fn hip_launch_plan_passes_explicit_u32_abi_for_32_bit_programs() {
    let program = SearchProgram::try_from_fixture("xor").unwrap();
    let plan = DriverCommandPlan::for_launch(
        &GpuSdk::Hip {
            sdk: "AMD HIP runtime".to_owned(),
        },
        &program,
        SearchDomain::new(0x50, 0x60),
        LaunchConfig {
...
#[test]
fn opencl_launch_plan_passes_explicit_u32_abi_for_32_bit_programs() {
    let program = SearchProgram::try_from_fixture("xor").unwrap();
    let plan = DriverCommandPlan::for_launch(
        &GpuSdk::OpenCl {
...
#[test]
fn process_driver_runner_recompiles_invalid_vulkan_cached_artifact() {
    let program = SearchProgram::try_from_fixture("xor").unwrap();
    let sdk = GpuSdk::Vulkan {
        sdk: "Vulkan runtime".to_owned(),
    };
    let output_dir = std::env::temp_dir().join(format!(
        "atlas-gpu-invalid-vulkan-cache-{}",
        std::process::id()
    ));
    let output_dir_text = output_dir.to_string_lossy().into_owned();
    let plan = DriverCommandPlan::for_sdk(&sdk, &program, &output_dir_text);
...
#[test]
fn process_driver_runner_recompiles_vulkan_cached_artifact_with_prefixed_entry_name() {
    let program = SearchProgram::try_from_fixture("xor").unwrap();
    let sdk = GpuSdk::Vulkan {
        sdk: "Vulkan runtime".to_owned(),
    };
    let output_dir = std::env::temp_dir().join(format!(
        "atlas-gpu-vulkan-cache-prefixed-entry-{}",
        std::process::id()
    ));
    let output_dir_text = output_dir.to_string_lossy().into_owned();
    let plan = DriverCommandPlan::for_sdk(&sdk, &program, &output_dir_text);
    fs::create_dir_all(Path::new(&plan.artifact_file).parent().unwrap()).unwrap();
    fs::write(&plan.artifact_file, b"\x03\x02\x23\x07atlas_search_extra").unwrap();
    let runner = RecordingCommandRunner::new();
...
#[test]
fn process_driver_runner_recompiles_invalid_opencl_cached_artifact() {
    let program = SearchProgram::try_from_fixture("xor").unwrap();
    let sdk = GpuSdk::OpenCl {
        sdk: "OpenCL runtime".to_owned(),
    };
    let output_dir = std::env::temp_dir().join(format!(
        "atlas-gpu-invalid-opencl-cache-{}",
        std::process::id()
    ));
    let output_dir_text = output_dir.to_string_lossy().into_owned();
    let plan = DriverCommandPlan::for_sdk(&sdk, &program, &output_dir_text);
    fs::create_dir_all(Path::new(&plan.artifact_file).parent().unwrap()).unwrap();
    fs::write(&plan.artifact_file, b"not opencl source").unwrap();
    let runner = RecordingCommandRunner::new();
...
#[test]
fn process_driver_runner_recompiles_opencl_cached_artifact_with_prefixed_entry_name() {
    let program = SearchProgram::try_from_fixture("xor64").unwrap();
    let sdk = GpuSdk::OpenCl {
        sdk: "OpenCL runtime int64".to_owned(),
    };
    let output_dir = std::env::temp_dir().join(format!(
        "atlas-gpu-opencl-cache-prefixed-entry-{}",
        std::process::id()
    ));
    let output_dir_text = output_dir.to_string_lossy().into_owned();
    let plan = DriverCommandPlan::for_sdk(&sdk, &program, &output_dir_text);
    ...
    fs::write(
        &plan.artifact_file,
        b"__kernel void atlas_search_extra() {}",
    )
    .unwrap();
    let runner = RecordingCommandRunner::new();
...
#[test]
fn process_driver_runner_recompiles_cuda_cached_artifact_with_prefixed_entry_name() {
    let program = SearchProgram::try_from_fixture("xor64").unwrap();
    let sdk = GpuSdk::Cuda {
        sdk: "CUDA runtime int64".to_owned(),
    };
    let output_dir = std::env::temp_dir().join(format!(
        "atlas-gpu-cuda-cache-prefixed-entry-{}",
        std::process::id()
    ));
    let output_dir_text = output_dir.to_string_lossy().into_owned();
    let plan = DriverCommandPlan::for_sdk(&sdk, &program, &output_dir_text);
    ...
    fs::write(
        &plan.artifact_file,
        b".visible .entry atlas_search_extra() {}",
    )
    .unwrap();
    let runner = RecordingCommandRunner::new();
...
#[test]
fn process_driver_runner_recompiles_invalid_hip_cached_artifact() {
    let program = SearchProgram::try_from_fixture("xor").unwrap();
    let sdk = GpuSdk::Hip {
        sdk: "HIP runtime".to_owned(),
    };
    let output_dir = std::env::temp_dir().join(format!(
        "atlas-gpu-invalid-hip-cache-{}",
        std::process::id()
    ));
    let output_dir_text = output_dir.to_string_lossy().into_owned();
    let plan = DriverCommandPlan::for_sdk(&sdk, &program, &output_dir_text);
...
#[test]
fn runtime_reports_cancellation_before_small_domain_completeness_audit() {
    let program = SearchProgram::new(
        16,
        vec![SearchOp::ChecksumEq {
            modulus: 1,
    ...
    let runner = CancellingSuccessfulRunner {
        token: token.clone(),
    };
...
#[test]
fn runtime_honors_cancellation_after_empty_gpu_launch() {
    let program = SearchProgram::new(
        8,
        vec![
            SearchOp::XorEq { mask: 0, target: 0 },
    ...
    let runner = CancellingEmptyRunner {
        token: token.clone(),
    };
...
#[test]
fn path_detected_runtime_ignores_invalid_vulkan_persisted_kernel_cache_for_warmed_gpu_threshold() {
    let tool_dir = std::env::temp_dir().join(format!(
        "atlas-gpu-invalid-vulkan-host-cache-{}",
        std::process::id()
    ));
    fs::create_dir_all(&tool_dir).unwrap();
    ...
    let cached_plan =
        DriverCommandPlan::for_launch(&sdk, &program, domain, launch, "target/atlas-gpu");
    fs::create_dir_all(Path::new(&cached_plan.artifact_file).parent().unwrap()).unwrap();
    fs::write(&cached_plan.artifact_file, b"not spirv").unwrap();
    let artifact_root = Path::new(&cached_plan.artifact_file)
        .parent()
        .unwrap()
        .to_path_buf();
...
#[test]
fn runtime_falls_back_when_match_count_is_less_than_retained_matches() {
    let program = SearchProgram::try_from_fixture("xor").unwrap();
    let token = CancellationToken::new();
    let sdk = GpuSdk::Vulkan {
        sdk: "test Vulkan".to_owned(),
    ...
    let runner = FixtureDriverRunner {
        output: DriverRunOutput {
            exit_code: 0,
            reported_matches: vec![0x55, 0x155],
    ...
    assert!(report
        .telemetry
        .rationale
        .contains("match_count 1 is less than retained matches 2"));
}

#[test]
fn runtime_falls_back_when_driver_reports_conflicting_match_counts() {
    let program = SearchProgram::try_from_fixture("xor").unwrap();
    let token = CancellationToken::new();
    let sdk = GpuSdk::Cuda {
        sdk: "test CUDA".to_owned(),
    ...
    assert!(report
        .telemetry
        .rationale
        .contains("conflicting match_count values 2 and 1"));
}
...
#[test]
fn runtime_falls_back_when_driver_reports_malformed_match_line() {
    let program = SearchProgram::try_from_fixture("xor").unwrap();
    let token = CancellationToken::new();
    let sdk = GpuSdk::Cuda {
        sdk: "test CUDA".to_owned(),
    ...
    assert_eq!(
        report.matches,
        NativeSearcher::search(&program, SearchDomain::new(0x50, 0x60), &token)
    );
    assert!(report
        .telemetry
        .rationale
        .contains("malformed match value not-a-number"));
}
